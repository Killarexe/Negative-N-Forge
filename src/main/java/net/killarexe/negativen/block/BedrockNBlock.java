...
public class BedrockNBlock extends NegativenModElements.ModElement {
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("bedrock_n", "bedrock_n", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.BEDROCK.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 22)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(22, 0, 0, 5))));
		}
	}
}
