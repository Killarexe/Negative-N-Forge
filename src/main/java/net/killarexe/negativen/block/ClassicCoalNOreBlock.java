...
public class ClassicCoalNOreBlock extends NegativenModElements.ModElement {
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("classic_coal_n_ore", "classic_coal_n_ore", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == ClassicStoneNBlock.block.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 17)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(20, 0, 0, 125))));
		}
	}
}
