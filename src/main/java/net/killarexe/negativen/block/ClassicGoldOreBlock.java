...
public class ClassicGoldOreBlock extends NegativenModElements.ModElement {
	@ObjectHolder("negativen:classic_gold_ore")
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("classic_gold_ore", "classic_gold_ore", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == ClassicStoneBlock.block.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 17)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(2, 0, 0, 32))));
		}
	}
}
