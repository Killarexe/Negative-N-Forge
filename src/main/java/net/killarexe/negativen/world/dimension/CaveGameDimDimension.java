...
public class CaveGameDimDimension extends NegativenModElements.ModElement {
	@ObjectHolder("negativen:cave_game_dim")
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("cave_game_dim"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("negativen:cave_game_dim")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("negativen:cave_game_dim"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("negativen:cave_game_dim"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("negativen:cave_game_biome")),};
	}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new OverworldGenSettings() {
				public BlockState getDefaultBlock() {
					return CobblestoneCaveGameBlock.block.getDefaultState();
				}
			...
			if (!biomesPatched) {
				for (Biome biome : this.biomes) {
					biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
						{
							carvableBlocks = ImmutableSet.of(CobblestoneCaveGameBlock.block.getDefaultState().getBlock(),
