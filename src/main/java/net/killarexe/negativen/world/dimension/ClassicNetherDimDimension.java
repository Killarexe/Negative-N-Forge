...
public class ClassicNetherDimDimension extends NegativenModElements.ModElement {
	@ObjectHolder("negativen:classic_nether_dim")
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("classic_nether_dim"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("negativen:classic_nether_dim")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("negativen:classic_nether_dim"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("negativen:classic_nether_dim"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("negativen:classic_nether")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new ClassicNetherDimItem().setRegistryName("classic_nether_dim"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("classic_nether_dim_portal");
		}
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = ClassicNetherDimDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
		...
		public static class Size {
			...
			protected int getDistanceUntilEdge(BlockPos pos, Direction directionIn) {
				...
				return (this.world.getBlockState(framePos).getBlock() == Blocks.OBSIDIAN.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == Blocks.OBSIDIAN.getDefaultState().getBlock())) {
								break label56;
							}
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("classic_nether_dim_portal", Vec3i::compareTo, 300);
	@SubscribeEvent
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "classic_nether_dim_portal",
	...
	public static class TeleporterDimensionMod implements ITeleporter {
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = ClassicNetherDimDimension.CustomPortalBlock.createPatternHelper(this.world,
						blockpos);
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, Blocks.OBSIDIAN.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
