...
public class NegaDimension extends NegativenModElements.ModElement {
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("nega"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("negativen:nega")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("negativen:nega"), dimension, null, true);
		}
		type = DimensionType.byName(new ResourceLocation("negativen:nega"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("negativen:negative")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("negativen:desert_n")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("negativen:oak_nforest")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("negativen:birch_n_forest")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("negativen:plains_n")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("negativen:dark_oak_n_forest")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("negativen:jungle_n")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("negativen:spruce_forest_n")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("negativen:classic_biome_n")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("negativen:montains_n")),};
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = NegaDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
			...
			protected int getDistanceUntilEdge(BlockPos pos, Direction directionIn) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == PortalblockBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = pos.offset(directionIn, i);
				return (this.world.getBlockState(framePos).getBlock() == PortalblockBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == PortalblockBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("nega_portal", Vec3i::compareTo, 300);
	@SubscribeEvent
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "nega_portal",
	...
	public static class TeleporterDimensionMod implements ITeleporter {
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = NegaDimension.CustomPortalBlock.createPatternHelper(this.world, blockpos);
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutable,
									flag ? PortalblockBlock.block.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, PortalblockBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new OverworldGenSettings() {
				public BlockState getDefaultBlock() {
					return StoneNBlock.block.getDefaultState();
				}

				public BlockState getDefaultFluid() {
					return WaterNBlock.block.getDefaultState();
				}
			});
			...
			if (!biomesPatched) {
				for (Biome biome : this.biomes) {
					biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
						{
							carvableBlocks = ImmutableSet.of(StoneNBlock.block.getDefaultState().getBlock(),
