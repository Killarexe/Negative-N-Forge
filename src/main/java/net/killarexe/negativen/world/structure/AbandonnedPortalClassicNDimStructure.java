...
import net.killarexe.negativen.world.dimension.StartDimDimension;
...
public class AbandonnedPortalClassicNDimStructure extends NegativenModElements.ModElement {
	public AbandonnedPortalClassicNDimStructure(NegativenModElements instance) {
		super(instance, 619);
	}
		...
		Feature<NoFeatureConfig> feature = new Feature<NoFeatureConfig>(NoFeatureConfig::deserialize) {
			...
			public boolean place(IWorld world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
				...
				if (dimensionType == MineingDimDimension.type)
					dimensionCriteria = true;
				if (dimensionType == StartDimDimension.type)
					dimensionCriteria = true;
				if (dimensionType == NoneDimDimension.type)
					dimensionCriteria = true;
				...
				if ((random.nextInt(1000000) + 1) <= 750) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) world.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("negativen", "abandonned_portal_classic_n_dim"));
						if (template == null)
							return false;
