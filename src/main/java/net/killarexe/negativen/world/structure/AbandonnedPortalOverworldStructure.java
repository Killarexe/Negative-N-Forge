...
public class AbandonnedPortalOverworldStructure extends NegativeNModElements.ModElement {
	private static Feature<NoFeatureConfig> feature = null;
	private static ConfiguredFeature<?, ?> configuredFeature = null;
	public AbandonnedPortalOverworldStructure(NegativeNModElements instance) {
		super(instance, 681);
	...
	private static class FeatureRegisterHandler {
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
					...
					if ((random.nextInt(1000000) + 1) <= 750) {
						...
						for (int a = 0; a < count; a++) {
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("negative_n", "abandonned_portal_3"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("abandonned_portal_overworld"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("negative_n:abandonned_portal_overworld"),
					configuredFeature);
		}
	}
	@SubscribeEvent
