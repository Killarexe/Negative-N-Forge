...
import net.killarexe.negativen.block.WarpedWartNBlockBlock;
import net.killarexe.negativen.block.WarpedNStemBlock;
import net.killarexe.negativen.block.WapredNyliumNBlock;
...
public class NetherNWarpedBiomeBiome extends NegativenModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.NONE)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(WapredNyliumNBlock.block.getDefaultState(),
							NetherrackNBlock.block.getDefaultState(), NetherrackNBlock.block.getDefaultState())));
			setRegistryName("nether_n_warped_biome");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(WarpedNStemBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(WarpedWartNBlockBlock.block.getDefaultState()))).baseHeight(7)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == WapredNyliumNBlock.block.getDefaultState().getBlock()
							|| ground == NetherrackNBlock.block.getDefaultState().getBlock())
							&& (ground2 == WapredNyliumNBlock.block.getDefaultState().getBlock()
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == WarpedWartNBlockBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, WarpedWartNBlockBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, WarpedNStemBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == WarpedNStemBlock.block.getDefaultState().getBlock()
					|| blockType == WarpedWartNBlockBlock.block.getDefaultState().getBlock()
					|| blockType == WapredNyliumNBlock.block.getDefaultState().getBlock()
