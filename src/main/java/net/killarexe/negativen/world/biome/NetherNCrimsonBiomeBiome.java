...
import net.killarexe.negativen.block.NetherWartNBlockBlock;
import net.killarexe.negativen.block.CrimsonNyliumNBlock;
import net.killarexe.negativen.block.CrimsonNStemBlock;
...
public class NetherNCrimsonBiomeBiome extends NegativenModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.NONE)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(CrimsonNyliumNBlock.block.getDefaultState(),
							NetherrackNBlock.block.getDefaultState(), NetherrackNBlock.block.getDefaultState())));
			setRegistryName("nether_n_crimson_biome");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(CrimsonNStemBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(NetherWartNBlockBlock.block.getDefaultState()))).baseHeight(7)
									.setSapling((net.minecraftforge.common.IPlantable) Blocks.JUNGLE_SAPLING).build())
					.withPlacement(Placement.COUNT_EXTRA_HEIGHTMAP.configure(new AtSurfaceWithExtraConfig(4, 0.1F, 1))));
			this.addSpawn(EntityClassification.MONSTER, new Biome.SpawnListEntry(EndermanNEntity.entity, 11, 1, 1));
		}
	}
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == CrimsonNyliumNBlock.block.getDefaultState().getBlock()
							|| ground == NetherrackNBlock.block.getDefaultState().getBlock())
							&& (ground2 == CrimsonNyliumNBlock.block.getDefaultState().getBlock()
									|| ground2 == NetherrackNBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), NetherrackNBlock.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == NetherWartNBlockBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, NetherWartNBlockBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, CrimsonNStemBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == CrimsonNStemBlock.block.getDefaultState().getBlock()
					|| blockType == NetherWartNBlockBlock.block.getDefaultState().getBlock()
					|| blockType == CrimsonNyliumNBlock.block.getDefaultState().getBlock()
					|| blockType == NetherrackNBlock.block.getDefaultState().getBlock();
		}
