...
public class DesertNBiome extends NegativenModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0f).depth(0.1f).scale(0.2f).temperature(1.5f).precipitation(Biome.RainType.NONE)
					.category(Biome.Category.DESERT).waterColor(-205).waterFogColor(-205)
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(SandNBlock.block.getDefaultState(),
							SandstoneNBlock.block.getDefaultState(), SandstoneNBlock.block.getDefaultState())));
			setRegistryName("desert_n");
			...
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(EndermanNEntity.entity, 11, 1, 15));
		}
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == SandNBlock.block.getDefaultState().getBlock() || ground == SandstoneNBlock.block.getDefaultState().getBlock())
							&& (ground2 == SandNBlock.block.getDefaultState().getBlock()
									|| ground2 == SandstoneNBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), SandstoneNBlock.block.getDefaultState(), bbox);
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, CactusNBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == CactusNBlock.block.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == SandNBlock.block.getDefaultState().getBlock()
					|| blockType == SandstoneNBlock.block.getDefaultState().getBlock();
		}
