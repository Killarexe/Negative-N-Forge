...
import net.killarexe.negativen.block.SpruceLeavesNBlock;
...
public class SpruceForestNBiome extends NegativenModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			...
			setRegistryName("spruce_forest_n");
			DefaultBiomeFeatures.addCarvers(this);
			DefaultBiomeFeatures.addStructures(this);
			DefaultBiomeFeatures.addMonsterRooms(this);
			DefaultBiomeFeatures.addOres(this);
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(SpruceLogNBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(SpruceLeavesNBlock.block.getDefaultState()))).baseHeight(10)
									.setSapling((net.minecraftforge.common.IPlantable) Blocks.JUNGLE_SAPLING).build())
					.withPlacement(Placement.COUNT_EXTRA_HEIGHTMAP.configure(new AtSurfaceWithExtraConfig(2, 0.1F, 1))));
			this.addSpawn(EntityClassification.MONSTER, new Biome.SpawnListEntry(ZombieNEntity.entity, 15, 1, 5));
			...
			this.addSpawn(EntityClassification.MONSTER, new Biome.SpawnListEntry(CreeperNEntity.entity, 15, 1, 5));
			this.addSpawn(EntityClassification.MONSTER, new Biome.SpawnListEntry(SpiderNEntity.entity, 15, 1, 5));
		}
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
				...
				if (!spawnTree) {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == SpruceLeavesNBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, SpruceLeavesNBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, SpruceLogNBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == SpruceLogNBlock.block.getDefaultState().getBlock()
					|| blockType == SpruceLeavesNBlock.block.getDefaultState().getBlock()
